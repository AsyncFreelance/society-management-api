...
const transferWallet = async (transactionInput: TransactionInput): Promise<TransactionReport> => {
  const foundPayerWallet = await WalletModel.findOne({ _id: transactionInput.from })
  if (!foundPayerWallet) {
    return formatError({ type: 400, message: "Payer Wallet does not exist" }) as any
  }

  const foundReceiverWallet = await WalletModel.findOne({ _id: transactionInput.to })
  ...
  // Debiting
  await WalletModel.updateOne({ _id: transactionInput.from }, { amount: Number(foundPayerWallet.amount - transactionInput.totalAmount) })
    .catch(err => formatError({ type: 500, message: String(err) }))

  // Crediting
  await WalletModel.updateOne({ _id: transactionInput.to }, { amount: Number(foundReceiverWallet.amount + transactionInput.amount) })
    .catch(err => formatError({ type: 500, message: String(err) }))
